...
class LunarLanderTerrainDataEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self):
        ...
        self.dHeight = [0 for i in range(CHUNKS)]

        # useful range is -1 .. +1, but spikes can be higher
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(8+CHUNKS,), dtype=np.float32)
    ...
    def reset(self):
        ...
        for i in range(CHUNKS-1):
            ...
            self.dHeight[i] = smooth_y[i] - smooth_y[i+1]
    ...
    def step(self, action):
        ...
        for dH in self.dHeight:
            state.append(dH)
        assert len(state) == 8 + len(self.dHeight)
